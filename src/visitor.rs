...
pub trait Visitor {
    ...
    fn visit_pragma(&mut self, pragma: &PragmaDirective) -> Result<(), Self::Error> {
        let loc = match pragma {
    ...
    fn visit_version(&mut self, loc: Loc, _version: &VersionComparator) -> Result<(), Self::Error> {
        self.visit_source(loc)
    }
...
/// All [`ast`] types, such as [Statement], should implement the [Visitable] trait

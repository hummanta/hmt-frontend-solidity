...
//! Heavily based on or copied from [solang-parser][solang-parser].
//!
...
pub enum Loc {
    ...
    /// Codegen
    Codegen,
    /// The file number, start offset and
    /// end offset in bytes of the source file.
...
impl Identifier {
    ...
    pub fn new(s: impl Into<String>) -> Self {
        Self { loc: Loc::default(), name: s.into() }
    }
}
...
impl UserDefinedOperator {
    ...
    pub const fn is_bitwise(&self) -> bool {
        matches!(self, Self::BitwiseAnd | Self::BitwiseOr | Self::BitwiseXor | Self::BitwiseNot)
    }

    /// Returns whether `self` is an arithmetic operator.
    #[inline]
    pub const fn is_arithmetic(&self) -> bool {
        matches!(self, Self::Add | Self::Subtract | Self::Multiply | Self::Divide | Self::Modulo)
    }
...
impl Expression {
    ...
    /// let pre_increment =
    ///     Expression::Add(Loc::default(), Box::new(var_a.clone()), Box::new(var_b.clone()));

...
impl Display for ast::Statement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        match self {
            Self::Block { unchecked, statements, .. } => {
                if *unchecked {
            ...
            Self::Assembly { dialect, flags, block, .. } => {
...
impl Display for ast::Type {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        match self {
            ...
            Self::Mapping { key, key_name, value, value_name, .. } => {
                f.write_str("mapping(")?;
            ...
            Self::Function { params, attributes, returns } => {
                f.write_str("function (")?;
...
mod tests {
    ...
    fn id(s: &str) -> ast::Identifier {
        ast::Identifier { loc: loc!(), name: s.to_string() }
    }

    macro_rules! yid {
        ($i:ident) => {
            ast::YulTypedIdentifier { loc: loc!(), id: id(stringify!($i)), ty: None }
        };
    ...
    fn yul_block() -> ast::YulBlock {
        ast::YulBlock { loc: loc!(), statements: vec![] }
    }

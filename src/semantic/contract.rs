    ...
    fn visit_sema_source_unit(&mut self, source_unit: &mut SourceUnit) -> Result<(), Self::Error> {
        ...
        for contract in &source_unit.contracts {
            for base in &contract.base {
                if self.ctx.contracts[contract.contract_no].is_library() {
                    self.ctx.diagnostics.push(Diagnostic::error(
                        base.loc,
                        format!(
                            "library '{}' cannot have a base contract",
                            self.ctx.contracts[contract.contract_no].id
                        ),
                    ));
                    continue;
                }
                    ...
                    self.ctx.resolve_contract_with_namespace(self.no, name, &mut diagnostics)
                {
                    ...
                    } else if self.ctx.contracts[contract.contract_no]
                        .bases
                        .iter()
                        .any(|e| e.contract_no == no)
                    {
                        self.ctx.diagnostics.push(Diagnostic::error(
                            ...
                            format!(
                                ...
                                self.ctx.contracts[contract.contract_no].id, name
                            ),
                        ));
                    } else if self.is_base(contract.contract_no, no) {
                        self.ctx.diagnostics.push(Diagnostic::error(
                            name.loc,
                            format!(
                                "base '{}' from contract '{}' is cyclic",
                                name, self.ctx.contracts[contract.contract_no].id
                            ),
                        ));
                    } else if self.ctx.contracts[contract.contract_no].is_interface() &&
                        !self.ctx.contracts[no].is_interface()
                    {
                        self.ctx.diagnostics.push(Diagnostic::error(
                            ...
                            format!(
                                ...
                                self.ctx.contracts[contract.contract_no].id,
                                self.ctx.contracts[no].ty,
                                name
                            ),
                        ));
                    } else if self.ctx.contracts[no].is_library() {
                        let contract = &self.ctx.contracts[contract.contract_no];
                        ...
                        self.ctx.contracts[contract.contract_no].bases.push(Base {
                            loc: base.loc,
                            contract_no: no,
                            constructor: None,
                        });

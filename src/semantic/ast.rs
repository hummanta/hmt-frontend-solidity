...
pub enum Type {
    ...
    /// The usize is an index into enums in the Context
    Enum(usize),
    /// The usize is an index into contracts in the Context
    Struct(StructType),
    Mapping(Mapping),
    /// The usize is an index into contracts in the Context
    Contract(usize),
    ...
    /// is an index into user_types in the Context.
    UserType(usize),
...
impl Type {
    ...
    pub fn unwrap_user_type(self, ctx: &Context) -> Type {
        if let Type::UserType(type_no) = self {
            ctx.user_types[type_no].ty.clone()
        } else {
            self
        }
    }
...
impl EventDecl {
    pub fn symbol_name(&self, ctx: &Context) -> String {
        match &self.contract {
            Some(c) => format!("{}.{}", ctx.contracts[*c].id, self.id),
...
impl ErrorDecl {
    pub fn symbol_name(&self, ctx: &Context) -> String {
        match &self.contract {
            Some(c) => format!("{}.{}", ctx.contracts[*c].id, self.name),
...
impl Function {
    ...
    /// `Context::function_externally_callable()` instead.
...
impl Symbol {
    ...
    pub fn has_accessor(&self, ctx: &Context) -> bool {
        if let Symbol::Variable(_, Some(contract_no), var_no) = self {
            matches!(
                ctx.contracts[*contract_no].variables[*var_no].visibility,
    ...
    pub fn is_private_variable(&self, ctx: &Context) -> bool {
        match self {
            Symbol::Variable(_, Some(contract_no), var_no) => {
                let visibility = &ctx.contracts[*contract_no].variables[*var_no].visibility;
...
impl Contract {
    ...
    pub fn constructor_needs_arguments(&self, ctx: &Context) -> bool {
        !self.constructors(ctx).is_empty() && self.no_args_constructor(ctx).is_none()
    }

    /// Does the contract have a constructor defined?
    /// Returns all the constructor function numbers if any
    pub fn constructors(&self, ctx: &Context) -> Vec<usize> {
        self.functions
            .iter()
            .copied()
            .filter(|func_no| ctx.functions[*func_no].is_constructor())
            .collect::<Vec<usize>>()
    }

    /// Return the constructor with no arguments
    pub fn no_args_constructor(&self, ctx: &Context) -> Option<usize> {
        self.functions
            .iter()
            .find(|func_no| {
...
/// This enum manages the accounts in an external call on Solana. There can be three optioctx:
    ...
    fn recurse<T>(&self, cx: &mut T, f: fn(expr: &Expression, ctx: &mut T) -> bool) {
        if f(self, cx) {
            match self {
                ...
                Expression::Subscript { array: left, index: right, .. } => {
                    left.recurse(cx, f);
                    right.recurse(cx, f);
                }
                ...
                Expression::ExternalFunctionCall { function, args, call_args, .. } => {
                    for e in args {
                        e.recurse(cx, f);
                    }
                    function.recurse(cx, f);
                ...
                Expression::ExternalFunctionCallRaw { address, args, call_args, .. } => {
                    args.recurse(cx, f);
                    address.recurse(cx, f);
                    call_args.recurse(cx, f);
                }
...
//             Instr::MemCopy { source, destination, .. } => match source.loc() {
